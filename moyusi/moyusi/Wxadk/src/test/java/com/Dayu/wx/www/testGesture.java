...
public class testGesture {
    ...
    //向左右上下滑动屏幕
    public void swipeTo(String actualparameter, AndroidDriver<WebElement> driver) throws InterruptedException {
    	...
    	case "向左滑动":
    		Thread.sleep(3000);
    		  new TouchAction(driver).press(fisrtpage,width*3/4,height*3/10).waitAction(Duration.ofMillis(500)).moveTo(fisrtpage,width/4,height*3/10).release().perform();
    	//	driver.swipe(width*9/10, height / 2, width/10, height / 2, 3000);
    		Thread.sleep(1000);
    		break;
    	case "购物车向左滑动":
    		Thread.sleep(3000);
    		  new TouchAction(driver).press(fisrtpage,width*3/4,height*2/10).waitAction(Duration.ofMillis(1000)).moveTo(fisrtpage,width/4,height*2/10).release().perform();
    	//	driver.swipe(width*9/10, height / 2, width/10, height / 2, 3000);
    		Thread.sleep(2000);
    		break;
    	case "向右滑动":
    		Thread.sleep(3000);
    		  new TouchAction(driver).press(fisrtpage,width/4,height*3/10).waitAction(Duration.ofMillis(500)).moveTo(fisrtpage,width*3/4,height*3/10).release().perform();
    	//	driver.swipe(width/10, height / 2, width*9/10, height / 2, 3000);
    		Thread.sleep(1000);
    		break;
    	case "向上滑动":
    		Thread.sleep(3000);
  		  new TouchAction(driver).press(fisrtpage,width/2,height*3/4).waitAction(Duration.ofMillis(1000)).moveTo(fisrtpage,width/2,height/4).release().perform();
    ...
    //向下滑动查找到某元素为止，可以翻页查找
    public static void swipeToelement(String actualelement, AndroidDriver<WebElement> driver,String actualparameter,List<WebElement> bot) throws InterruptedException {
    	width = driver.manage().window().getSize().width;
        height = driver.manage().window().getSize().height;
    	boolean a = false;
    	int counter = 0;
    ...
    public static void swipeToelementAllD(String actualelement, AndroidDriver<WebElement> driver,String actualparameter,List<WebElement> bot,String mode) throws InterruptedException {
    	width = driver.manage().window().getSize().width;
        height = driver.manage().window().getSize().height;
        WebElement fisrtpage=driver.findElementByClassName("android.widget.FrameLayout");
    	boolean a = false;
    ...
    public static void swipeAndClick(String actualelement, AndroidDriver<WebElement> driver,String actualparameter,String direction) throws InterruptedException 
    {
    	width = driver.manage().window().getSize().width;
        height =  driver.manage().window().getSize().height;
