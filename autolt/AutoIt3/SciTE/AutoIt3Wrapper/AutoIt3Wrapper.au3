#NoTrayIcon
Global Const $VERSION = "18.708.1148.0"
...
#AutoIt3Wrapper_Res_Fileversion=18.708.1148.0
#AutoIt3Wrapper_Res_Fileversion_AutoIncrement=p
#AutoIt3Wrapper_Res_Fileversion_First_Increment=y
#AutoIt3Wrapper_Res_ProductName=AutoIt3Wrapper Production
#AutoIt3Wrapper_Res_ProductVersion=2.2
#AutoIt3Wrapper_Res_CompanyName=Jos van der Zande
#AutoIt3Wrapper_Res_LegalCopyright=Copyright © 2018 Jos van der Zande
...
For $i = 1 To $aList[0][0]
	...
	If StringInStr($cmdlineraw, $CurrentFile) Then
		; When the currentfilename is the same as the file being processed by AutoIt3wrapper. then select the handle
...
If Not StringInStr($CMDLINERAW, "/watcher") _
	...
	__ConsoleWrite("+>" & @HOUR & ":" & @MIN & ":" & @SEC & " Starting AutoIt3Wrapper v." & $VERSION)
	If FileExists($SciTE_Dir & "\SciTE.exe") Then __ConsoleWrite(' SciTE v.' & FileGetVersion($SciTE_Dir & "\SciTE.exe") & " ")
	__ConsoleWrite("  Keyboard:" & @KBLayout & "  OS:" & @OSVersion & "/" & @OSServicePack & "  CPU:" & @CPUArch & " OS:" & @OSArch)
...
Global $INP_Res_Language = "", $INP_Res_requestedExecutionLevel = "", $INP_RES_HiDpi = "", $INP_Res_Compatibility = "", $INP_Fileversion = "", $INP_Fileversion_New = "", $INP_Fileversion_AutoIncrement = "", $INP_Fileversion_First_Increment = "", $INP_LegalCopyright = "", $INP_LegalTradeMarks = ""
...
;~ 			Just for compatibility sake
		...
		Case $T_Var = "/tidy"
			$Option = "Tidy"
		Case $T_Var = "/RunAfter"
			; This option is a workaround for not compiling utilities anymore and still wanting to copy the Script, updating version and running SVN commit.
			...
			For $x = 1 To $INP_Run_After[0]
				If StringStripWS($INP_Run_After[$x], 3) <> "" Then
					...
					Else
						__ConsoleWrite(">Running:" & $INP_Run_After[$x] & @CRLF)
			...
			If $INP_Fileversion_AutoIncrement = 'y' Then Update_Version()
...
If $INP_Run_Stop_OnError = "" Then $INP_Run_Before = StringSplit(IniRead($AutoIt3WapperIni, "Other", "Run_Stop_OnError", ""), "")
...
If $INP_AutoIt3Wrapper_Testing = "y" Then
	...
	;__ConsoleWrite("- *** Compile in Test mode skipping Tidy; Au3Check; Au3Stripper; Resource updating and Versioning to speed up the process. ***" & @CRLF)
EndIf
...
If $Option = "Compile" Then
	...
	For $x = 1 To $INP_Run_Before[0]
		If StringStripWS($INP_Run_Before[$x], 3) <> "" Then
			...
			Else
				__ConsoleWrite(">Running:" & $INP_Run_Before[$x] & @CRLF)
...
;~ 		__ConsoleWrite("! *************************************************************************************" & @CRLF)
	Else
		...
		If FileExists($Tidypgm) Then
			...
			__ConsoleWrite(">Running Tidy " & $TidypgmVer & "  from:" & $Tidypgmdir & @CRLF)
...
;~ 						Show_Warnings("Tidy errors", StringReplace($Return_Text, @CR, ""))
				...
				EndIf
			Else
				;__ConsoleWrite(">Tidy Ended. No Error(s).   rc:" & $exitcode & @crlf)
				Write_RC_Console_Msg("Tidy ended.", $ExitCode)
			EndIf
		Else
...
;~ 	__ConsoleWrite("-> Skipping AU3check: Current version doesn't support the AutoIt3 v 3.3.9.5+ syntax." & @LF)
...
Func CTRL_BREAK_WRAPPER()
	SendSciTE_Command($My_Hwnd, $SciTE_hwnd, "menucommand:304") ; IDM_STOPEXECUTE
...
Func Run_the_Script($Init = 0)
	...
	;
	; Set hotkeys for StopExecute and Restart
	...
	Local $sSTOP = ($INP_HotKey_Stop = '^{BREAK}') ? '^Break' : '^Break -or- ' & $INP_HotKey_Stop
	...
	$sSTOP = StringReplace($sSTOP, "}", "")
	Local $sRestart = ($INP_HotKey_Restart = '^!{BREAK}') ? '^!Break' : $INP_HotKey_Restart
	...
	$sRestart = StringReplace($sRestart, "}", "")
	__ConsoleWrite('--> Press ' & $sRestart & ' to Restart or ' & $sSTOP & ' to Stop' & @CRLF)
	;
...
;~ 		__ConsoleWrite("! ***************************************************************************************************" & @CRLF)
;~ 		__ConsoleWrite("! * Input file is UTF" & $UTFtype & " encoded, Au3Stripper do not support UNICODE and will be skipped.*" & @CRLF)
;~ 		__ConsoleWrite("! ***************************************************************************************************" & @CRLF)
	ElseIf $InputFileIsUTF16 Or $InputFileIsUTF32 Then
;~ 		__ConsoleWrite("! ***************************************************************************************************" & @CRLF)
;~ 		__ConsoleWrite("! * Input file is UTF" & $UTFtype & " encoded, Tidy and Au3Stripper do not support UNICODE and will be skipped.*" & @CRLF)
;~ 		__ConsoleWrite("! ***************************************************************************************************" & @CRLF)
	Else
...
;~ 						$Pragma_ = $PragmaOption[1]
			...
			EndIf
		EndIf
		; Show Consolemessge about running a script that required elevated rights not displaying anuy console messages
		If $Option = "Run" And $I_Rec = "#RequireAdmin" And Not IsAdmin() Then
			__ConsoleWrite("! Your script requires Admin rights while SciTE is running at normal level." & @CRLF)
			__ConsoleWrite("! This means no Console output from the script will be displayed and SciTE will show the script as ended right away." & @CRLF)
		EndIf
		;
...
;~ 	__ConsoleWrite('@@ ######## start Setdefaults : $fieldval = ' & $fieldval & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	...
	If $valid <> "" Then
		...
		EndIf
	EndIf
;~ 	__ConsoleWrite('@@ After validate: $fieldval = ' & $fieldval & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	If $iNbr Then $fieldval = Number($fieldval)
;~ 	__ConsoleWrite('@@ after number : $fieldval = ' & $fieldval & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
EndFunc   ;==>SetDefaults
;
