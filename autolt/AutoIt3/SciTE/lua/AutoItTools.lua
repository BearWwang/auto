...
function AutoItTools:OnStartup()
	...
	self.Pattern = "()[ /t][Ff][Uu][Nn][Cc][ /t]+([%w%s_]*%(.-%))"
...
--~ local from, to, name = line:find("[Ff][Uu][Nn][Cc][ /t]*([%w%s_]*)")
...
function AutoItTools:ConsoleWritePattern(with_comment)
	...
	if with_comment then
		return nl .. "[ /t]*;[ /t]*(ConsoleWrite%([^" .. nl .. "]-%) ;### Debug[^" .. nl .. "]+)"
	else
		return nl .. "[ /t]*(ConsoleWrite%([^" .. nl .. "]-%) ;### Debug[^" .. nl .. "]+)"
	end
...
function AutoItTools:MsgBoxPattern(with_comment)
	local nl = self:NewLineInUse()
	if with_comment then
		return nl .. "[ /t]*;[ /t]*(MsgBox%([^" .. nl .. "]-%) ;### Debug[^" .. nl .. "]+)"
	else
		return nl .. "[ /t]*(MsgBox%([^" .. nl .. "]-%) ;### Debug[^" .. nl .. "]+)"
	end
end	-- MsgBoxPattern()
...
function AutoItTools:FunctionTracePattern(with_comment)
	local nl = self:NewLineInUse()
	if with_comment then
		return nl .. "[ /t]*;[ /t]*(ConsoleWrite%([^" .. nl .. "]-%)[ /t]*;### Function Trace[^" .. nl .. "]?)"
	else
		return nl .. "[ /t]*(ConsoleWrite%([^" .. nl .. "]-%)[ /t]*;### Function Trace[^" .. nl .. "]?)"
	end
...
function AutoItTools:FunctionTraceAdd()
    -- Pattern to match
--~ local sPattern = "()([Ff][Uu][Nn][Cc][ /t]*([%w_]*)%(.-%))([^\r\n]*)"
    ...
    local function pat_match(m1, m2, m3, m4)
        if editor.StyleAt[m1] == SCE_AU3_COMMENT or
            ...
            m4:find(";[ /t]*[Ff][Uu][Nn][Cc][Tt][Ii][Oo][Nn][Tt][Rr][Aa][Cc][Ee][Ss][Kk][Ii][Pp]") then
            return m2 .. m4
        end
        i = i + 1
...
function AutoItTools:TracePattern(with_comment)
	local nl = self:NewLineInUse()
	if with_comment then
		return nl .. "[ /t]*;[ /t]*(ConsoleWrite%([^" .. nl .. "]-%) ;### Trace Console[^" .. nl .. "]?)"
	else
		return nl .. "[ /t]*(ConsoleWrite%([^" .. nl .. "]-%) ;### Trace Console[^" .. nl .. "]?)"
	end
end	-- TracePattern()
